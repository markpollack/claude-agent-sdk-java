...
import io.github.markpollack.claude.agent.sdk.mcp.McpMessageHandler;
...
import io.github.markpollack.claude.agent.sdk.streaming.BlockingMessageReceiver;
...
import io.github.markpollack.claude.agent.sdk.streaming.ResponseBoundedReceiver;
	...
	private ControlResponse handleCanUseTool(String requestId, ControlRequest.CanUseToolRequest canUseTool) {
		...
		try {
			...
			if (result.isAllowed()) {
				...
				// Always include updatedInput — Claude Code's Zod schema requires it
				response.put("updatedInput", allow.hasUpdatedInput() ? allow.updatedInput() : Map.of());
